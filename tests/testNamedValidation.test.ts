...
Deno.test("Test errorMessages state", async (test: Deno.TestContext) => {
	...
	assert(v$.state.age?.$state?.results != undefined, "Property state's results map was not defined immediately after calling the composable.");
	...
	assert(syncResult != undefined, "Synchronous named validation result did not appear in the results map.");
	assert(syncResult?.isValid === true, "Synchronous named validation result was not valid even though it should have been. If the isValid test is passing, this is likely a problem with assignment to the results map internally.");
	assert(syncResult?.custom?.passwordStrength === PassingValue, "Synchronous named validation result did not has password strength in its custom field.");
	await promise;
	const asyncResult = v$.state.age?.$state?.results[ValidationName2];
	assert(asyncResult != undefined, "Asynchronous named validation result did not appear in the results map.");
});