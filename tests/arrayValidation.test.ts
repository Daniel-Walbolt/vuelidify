...
const testPrimitiveArrayValidation = async (test: Deno.TestContext) => {
	...
	assert(
		isReactiveElementValidationPerformed.value === false,
		"Reactive array validation happened even though it was delayed and validate() was not invoked.",
	);
	assert(
		isLazyElementValidationPerformed.value === false,
		"Lazy validation happened even though validate() was not invoked.",
	);
	await v$.validate();
	assert(
		isReactiveElementValidationPerformed.value === true,
		"Reactive element validation was not performed even though validate() was invoked.",
	);
	assert(
		isLazyElementValidationPerformed.value === true,
		"Lazy element validation was not performed even though validate() was invoked.",
	);
	assert(
		reactiveValidationCount.value === 1,
		`Reactive Array validation happened ${reactiveValidationCount.value} times, but should have happened once.`,
	);
	assert(
		arrayValidationCount.value != 0,
		"Array element validation was not performed when it should have been. This is likely because the $arrayState computed function is not executed.",
	);
	assert(
		arrayValidationCount.value === model.value.length,
		`Validation of array elements happened ${arrayValidationCount.value} times, but should have happened ${model.value.length} times`,
	);
	...
	await pause();
	assert(
		reactiveValidationCount.value > 0,
		`Array validation did not happen reactively after assignment.`,
	);
	assert(
		v$.isValid === false,
		`Array validation passed when it should have failed because the element validator only passes "${PassingValue}".`,
	);
	// Make sure array state matches the length of the array and correctly validates each element.
	assert(
		v$.state.$arrayState.length === model.value.length,
		`Array state was not the same length of the model (${v$.state.$arrayState.length} instead of ${model.value.length}).`,
	);
	assert(
		v$.state.$arrayState[0].$state?.isValid == true,
		`Array state 0 was not valid even though it should have been.`,
	);
	assert(
		v$.state.$arrayState[1].$state?.isValid === false,
		`Array state 1 was valid even though it should NOT have been.`,
	);
	...
	assert(
		v$.state.$arrayState[0].$state?.isValid === true,
		`Array state 0 was invalid when it should have been valid (lazy validation).`,
	);
	assert(
		v$.state.$arrayState[1].$state?.isValid === false,
		`Array state 1 was valid when it should have bee invalid (lazy validation).`,
	);
	model.value.reverse();
	assert(
		v$.state.$arrayState[0].$state?.isValid === true,
		`Array state 0 was invalid when it should have stayed true. Shuffling an array of primitives does not reliably move validation state.`,
	);
	assert(
		v$.state.$arrayState[1].$state?.isValid === false,
		`Array state 1 was valid when it should have stayed false. Shuffling an array of primitives does not reliably move validation state.`,
	);
};
...
const testObjectArrayValidation = async (test: Deno.TestContext) => {
		...
		validation: {
			$each: {
				$lazy: [
					...
					(params) => {
						assert(
							model.value.includes(params.value),
							"The value provided to a lazy object validator was not in the model array.",
						);
						return {
							isValid: true,
						};
	...
	assert(
		isElementReactiveValidationPerformed.value === false,
		"Reactive array validation happened even though it was delayed and validate() was not invoked.",
	);
	assert(
		isElementLazyValidationPerformed.value === false,
		"Lazy validation happened even though validate() was not invoked.",
	);
	await v$.validate();
	assert(
	...
	assert(
		arrayValidationCount.value === model.value.length,
		`Element validation happened ${arrayValidationCount.value} times when it should have happened ${model.value} times.`,
	);
	// Make sure array state matches the length of the array and correctly validates each element.
	assert(
		v$.state.$arrayState.length === model.value.length,
	...
	model.value.reverse();
	assert(
		v$.state.$arrayState[0].$state?.isValid === false,
		`Array state 0 was valid after a reversal when it should have been invalid (lazy validation state did not follow array elements)`,
	);
	assert(
		v$.state.$arrayState[1].$state?.isValid === true,
		`Array state 1 was invalid after a reversal when it should have been valid (lazy validation state did not follow array elements)`,
	);
	model.value[1].name = PassingValue + "3";
	assert(
		v$.state.$arrayState[1].$state?.isValid === true,
		`Array state 1 was invalid after changing it to a non-valid value. The validation should be lazy, and not update reactively.`,
	);
};
...
const testArrayParentParameter = async (test: Deno.TestContext) => {
					...
					$lazy: [
						(params) => {
							...
							assert(
								...
								`Ancestor 0's array did not match the model's neighbor array ${
									isPostShuffle.value ? "(Post Shuffle)" : ""
								}.`,
							);
							...
							assert(
								...
								`Ancestor 0's array index (${
									params.arrayAncestors[0].index
								}) did not match the manually found index in the array (${calculatedIndex}) ${
									isPostShuffle.value ? "(Post Shuffle)" : ""
								}.`,
							);
							return {
	...
	assert(
		ranReactiveNeighborValidation.value === true,
		"Test did not run reactive neighbor validation when it should have.",
	);
...
const testDeeplyNestedArrayValidation = async (test: Deno.TestContext) => {
	...
	const repeatableValidation = (
		times: number,
	): ArrayValidation<Person, Person[]> | undefined => {
		if (times === 0) {
			return undefined;
		}
		return {
			$each: {
				...
				$lazy: [
					(params) => {
						...
						assert(
							validPathIndex >= 0,
							`Array path: ${displayPath} does not exist in the calculated possible ancestor paths.`,
						);
						return {
							isValid: true,
						};
	...
	assert(
		...
		"There were remaining unique ancestor paths in the generated data when there should have been zero. This means that validation was not exhaustive, and may not have the same depth as the data generated.",
	);
};
