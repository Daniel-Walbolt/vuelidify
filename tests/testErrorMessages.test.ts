...
Deno.test("Test errorMessages state", async (test: Deno.TestContext) => {
	...
	assert(
		v$.state.age?.$state?.errorMessages != undefined,
		"Property state's errorMessages was not defined immediately after calling the composable.",
	);
	...
	assert(
		v$.state.age?.$state?.errorMessages.includes(ErrorMessage2),
		"Error messages does not contain the expected error messages (ErrorMessage2).",
	);
	await promise;
	assert(
		v$.state.age?.$state?.errorMessages.includes(ErrorMessage3),
		"Error messages does not contain the expected error messages (ErrorMessage3).",
	);
	model.value.age = PassingValue * 2; // Fails all validations
	await pause(30);
	assert(
		v$.state.age?.$state?.errorMessages.includes(ErrorMessage1),
		"Error messages does not contain the expected error messages (ErrorMessage1)",
	);
	assert(
		v$.state.age?.$state?.errorMessages.includes(ErrorMessage3),
		"Error messages does not contain the expected error messages (ErrorMessage3). It should persist in the array because lazy validation was not invoked again.",
	);
	await pause(PromiseLengthMs1);
	assert(
		v$.state.age?.$state?.errorMessages.includes(ErrorMessage2) === false,
		"Error messages contains an error message it shouldn't (ErrorMessage2).",
	);
});
